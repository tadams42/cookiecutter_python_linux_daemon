...
class TestConfigLoader(ExternalConfigLoader):
    ...
    - config is never loaded from system wide locations (to prevent automatic loading
      of production config when starting application in test mode)
    - command line arguments for ``--config-file-path``, ``--log-file-path`` and
      ...
      automatic use of systemwide config files in test mode is dangerous this is
      prevented but stated exception.
    ...
    **Config and log file locations**
    ...
    - application log file path
        ...
        - ``/tmp/{{cookiecutter.project_slug}}.tmp/test.log`` if running from installed package
    """
